			...
			success:function(respData){				
				$("#orderModal").on("shown.bs.modal", function () { 
					...
					respData.data.forEach(function(item){	
						...
						if(itemCount == 1) {					
							$('#itemCategory_'+itemCount).val(item['category_id']);													
							...
							setTimeout(function () {								
								$('#itemCategory_'+count).val(item['category_id']);
...
function itemHTMLRows(count) {
	...
	htmlRows += '<td><input type="hidden" name="itemIds[]" id="itemIds_'+count+'" class="form-control"></td>'; 	
	htmlRows += '</tr>';
	return htmlRows;
}
...
function loadTableList(id) {	
	$.ajax({
		url:"order_action.php",
		method:"POST",
		data:{action:'loadTableList'},
		success:function(data) {
			$('#tableName_'+id).html(data);
		}
	});
}
